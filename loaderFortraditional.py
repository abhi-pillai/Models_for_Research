...
def load_data(path):
    ...
    for rec in records:
        ...
        for s, sym in zip(ann.sample, ann.symbol):
            ...
            feats.append(extract_features(beat))
            labels.append(y)
            p_ids.append(rec)

    return pd.DataFrame(feats), np.array(labels), np.array(p_ids)

# =========================
# PATIENT-WISE SPLIT
# =========================
def split_data(X, y, p_ids):
...
X_df, y, p_ids = load_data(DATA_PATH)

print("Splitting...")
X_tr, y_tr, X_va, y_va, X_te, y_te, tr_p, va_p, te_p = split_data(X_df.values, y, p_ids)
